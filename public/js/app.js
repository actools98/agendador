document.addEventListener('DOMContentLoaded', function () {
  ...
  function waitForFullCalendar() {
    return new Promise((resolve) => {
      ...
      // Si no está, esperar hasta que se cargue (hasta 10s)
      let attempts = 0;
      const interval = setInterval(() => {
        attempts++;
        if (typeof FullCalendar !== 'undefined' || attempts > 50) {
  ...
  waitForFullCalendar().then(() => {
    if (typeof FullCalendar === 'undefined') {
      console.error('❌ FullCalendar no se cargó después de esperar');
      document.getElementById('calendar').innerHTML = '<p class="text-danger">Error: No se pudo cargar el calendario. Intenta recargar la página.</p>';
      return;
    }
    ...
    try {
      ...
      calendarEl.innerHTML = '<p class="text-danger">Error al cargar el calendario. Revisa la consola.</p>';
    }
