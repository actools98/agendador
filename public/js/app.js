document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!mobilePanel && window.innerWidth < 768) {
    ...
    // Eliminar elementos que no queremos en el móvil (como el título de bienvenida en desktop)
  ...
  // Cerrar panel al hacer clic en un botón o enlace
  if (mobilePanel) {
    mobilePanel.addEventListener('click', function(e) {
      const target = e.target.closest('.btn, a');
      if (target) {
        // No cerrar si es un botón de edición o eliminación de eventos
  ...
  function syncMobileSidebarButtons() {
    ...
    // Logout
    const logoutBtnMobile = mobilePanel.querySelector('a[href="/logout"]');
    if (logoutBtnMobile) {
      logoutBtnMobile.addEventListener('click', function(e) {
        // No hacer nada especial, el enlace ya hace logout
      });
    }
  }
  ...
  toggleFinishedBtn.addEventListener('click', function() {
    ...
    finishedListEl.style.display = finishedVisible ? 'block' : 'none';
    this.textContent = finishedVisible ? '📋 Ocultar terminados' : '📋 Eventos terminados';
    // Sincronizar el botón del sidebar
    const desktopBtn = document.getElementById('toggleFinishedBtnDesktop');
    if (desktopBtn) {
      desktopBtn.textContent = this.textContent;
    }
  ...
  // Configuración desde el sidebar (desktop) - el mismo botón
